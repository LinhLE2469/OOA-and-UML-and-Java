...
public class PassengersDataService {
    ...
    public static List<Passenger> readPassengersFromFile(File file) {
        ...
        try (Scanner scanner = new Scanner(file)) {
            ...
            //define passengers and invalidPasseneger a list (array)
            List<Passenger> passengers = new ArrayList<>();

            //we want to skip the header row; so nextLine
            scanner.nextLine();
            // Scan line trong file data and put it in a array trong While loop
            while (scanner.hasNext()) {
                ...
                if (!invalidAge) {
                    //chuyen doi kieu string Age sang Double
                    age = Double.parseDouble(rawAge);
                }
                //add new passenger (call define tu Passenger Class) into passengers array
                Passenger passenger = new Passenger(passengerId, survived, pclass, gender, age);
                if (invalidAge) {
                    //do nothing or print a warning
                } else {
                    //neu co thong tin age thi add passenger do vao list passengers va tinh tuoi
